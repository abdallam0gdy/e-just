...
export default function StudentDashboard() {
  ...
  const captureNextSelfie = useCallback(() => {
    ...
    try {
      ...
      if (!img) throw new Error('فشل التقاط الصورة، تأكد من صلاحية الكاميرا');
  ...
  const submitAttendance = async () => {
    ...
    try {
      ...
      if (error) {
        if (error.code === '23505') throw new Error('لقد سجلت حضورك لهذه المحاضرة بالفعل!');
        throw error;
      }
      setMessage({ type: 'success', text: `تم تسجيل حضورك بنجاح${selectedLecture ? ' - ' + selectedLecture.title : ''}` });
      setStep('done');
      ...
      fetchAll();
    } catch (err) { setMessage({ type: 'error', text: err.message || 'فشل تسجيل الحضور.' }); }
    ...
    <div className="space-y-4 sm:space-y-5">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 sm:p-5 relative overflow-hidden">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2">
          <div>
            <h2 className="text-lg sm:text-xl font-bold text-navy-900 mb-0.5">مرحباً، {profile?.full_name}</h2>
            <p className="text-navy-400 text-xs sm:text-sm">منصة تسجيل الحضور الإلكتروني</p>
          </div>
          <span className="bg-navy-900 text-white font-bold text-[10px] sm:text-xs px-3 py-1 rounded-full">بوابة الطالب</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-5">
        ...
        <div className="lg:col-span-2">
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 sm:p-5">
            ...
            {(step === 'idle' || step === 'done') && !cameraOpen && (
              <div>
                <p className="text-xs text-navy-500 font-bold mb-3">📚 محاضرات اليوم — اختر المحاضرة لتسجيل حضورك:</p>
                {loadingLectures ? (
                  <div className="py-8 text-center"><div className="spinner mx-auto mb-2 !w-6 !h-6"></div><p className="text-xs text-navy-400">جاري تحميل المحاضرات...</p></div>
                ) : todayLectures.length === 0 ? (
                  <div className="text-center py-8 bg-gray-50 rounded-xl border-2 border-dashed border-gray-200">
                    <p className="text-sm font-bold text-navy-900 mb-1">لا توجد محاضرات اليوم</p>
                    <p className="text-navy-400 text-xs">لم يتم جدولة أي محاضرات لهذا اليوم.</p>
                  </div>
                ) : (
                          ...
                          <div className="flex items-center justify-between gap-2">
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center gap-2 mb-1">
                                ...
                                {/* Status badge */}
                                {attended ? (
                                  <span className="text-[9px] font-bold bg-green-100 text-green-700 px-1.5 py-0.5 rounded-full shrink-0">✓ تم التسجيل</span>
                                ) : status === 'active' ? (
                                  <span className="text-[9px] font-bold bg-green-50 text-green-600 px-1.5 py-0.5 rounded-full animate-pulse shrink-0">● نشطة</span>
                                ) : status === 'upcoming' ? (
                                  <span className="text-[9px] font-bold bg-blue-50 text-blue-600 px-1.5 py-0.5 rounded-full shrink-0">قادمة</span>
                                ) : (
            ...
            {step === 'camera' && cameraOpen && (
              <div className="space-y-3">
                ...
                <div className="flex items-center justify-between bg-navy-50 p-2 rounded-lg border border-navy-100">
                  ...
                  <button onClick={goBackToLectures} className="text-[10px] text-red-600 font-bold hover:underline">✕ رجوع</button>
                </div>
                {/* Step indicator */}
                <div className="flex items-center justify-center gap-2 text-xs font-bold">
                  <div className={`flex items-center gap-1 px-2.5 py-1 rounded-full ${!capturedImages.img1 ? 'bg-navy-900 text-white' : 'bg-green-100 text-green-700'}`}>{capturedImages.img1 ? '✓' : '1'} سيلفي</div>
                  <div className="w-6 h-0.5 bg-gray-300"></div>
                  <div className={`flex items-center gap-1 px-2.5 py-1 rounded-full ${capturedImages.img1 ? 'bg-navy-900 text-white' : 'bg-gray-100 text-gray-400'}`}>2 كارنيه</div>
                </div>
                <div className="relative max-w-md mx-auto bg-black rounded-xl overflow-hidden shadow-inner border-2 border-gray-800">
                  ...
                  <div className="absolute top-2 right-2 bg-black/60 text-white px-2 py-0.5 rounded-full text-[10px] font-bold flex items-center gap-1"><span className="w-1.5 h-1.5 rounded-full bg-red-500 animate-pulse"></span>{facingMode === 'user' ? 'أمامية' : 'خلفية'}</div>
                  <div className="absolute bottom-2 left-2 bg-navy-900/80 text-white px-2 py-0.5 rounded-full text-[10px] font-bold">{!capturedImages.img1 ? '📸 التقط سيلفي' : '🪪 صوّر الكارنيه'}</div>
                  {capturedImages.img1 && (
                    <div className="absolute bottom-2 right-2 border-2 border-white rounded-lg shadow-lg overflow-hidden w-14 h-14 bg-black"><img src={capturedImages.img1} alt="Selfie" className="w-full h-full object-cover" /><div className="absolute bottom-0 w-full bg-black/60 text-white text-[7px] text-center py-0.5 font-bold">سيلفي ✓</div></div>
                  )}
                </div>
                <div className="flex justify-center gap-2">
                  ...
                  <button onClick={goBackToLectures} className="bg-gray-100 hover:bg-gray-200 text-gray-700 font-bold py-2 px-4 rounded-lg text-xs">إلغاء</button>
                </div>
              </div>
            )}
              ...
              <div className="space-y-3">
                ...
                <div className="bg-navy-50/50 p-3 rounded-xl border border-navy-100">
                  <div className="grid grid-cols-2 gap-3">
                    <div className="bg-white p-2 rounded-lg shadow-sm border border-gray-100">
                      <p className="text-center text-[10px] font-bold text-navy-800 mb-2 bg-navy-50 py-1 rounded">📸 السيلفي</p>
                      <img src={capturedImages.img1} alt="Selfie" className="w-full rounded-lg object-cover" />
                    </div>
                    <div className="bg-white p-2 rounded-lg shadow-sm border border-gray-100">
                      <p className="text-center text-[10px] font-bold text-navy-800 mb-2 bg-navy-50 py-1 rounded">🪪 الكارنيه</p>
                ...
                <div className="flex justify-center gap-2">
                  <button onClick={submitAttendance} disabled={uploading} className="bg-green-600 hover:bg-green-700 disabled:bg-green-400 text-white font-bold py-2 px-4 rounded-lg flex items-center gap-1.5 text-xs">
                    {uploading ? (<><div className="spinner !w-4 !h-4 !border-2"></div>جاري الرفع...</>) : (<>✓ تأكيد وتسجيل</>)}
                  </button>
                  <button onClick={resetCapture} className="bg-gray-100 hover:bg-gray-200 text-gray-700 font-bold py-2 px-4 rounded-lg text-xs" disabled={uploading}>إعادة</button>
                </div>
              </div>
            )}
          </div>
        ...
        <div className="lg:col-span-1">
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 sm:p-5 h-full">
            ...
            {loadingLogs ? (
              <div className="py-8 text-center"><div className="spinner !w-6 !h-6 mx-auto mb-2"></div><p className="text-[11px] text-gray-500 font-bold">جاري التحميل...</p></div>
            ) : logs.length === 0 ? (
                ...
                {logs.map(log => (
                  <div key={log.id} className="p-2.5 border border-gray-100 rounded-lg bg-gray-50 hover:bg-white hover:shadow-sm transition-all">
                    <div className="flex justify-between items-start mb-1.5">
                      <div>
                        <p className="text-xs font-bold text-navy-900 flex items-center gap-1">
                          ...
                          {log.lectures?.title || 'محاضرة مسجلة'}
                        </p>
                    ...
                    <div className="flex gap-2 justify-center bg-white p-1.5 rounded border border-gray-100">
                      <div className="text-center"><img src={log.image_1_url} alt="Selfie" className="w-11 h-11 rounded-md bg-gray-100 object-cover border border-gray-200 shadow-sm" loading="lazy" /><span className="text-[8px] text-navy-400 font-bold">سيلفي</span></div>
                      <div className="text-center"><img src={log.image_2_url} alt="ID" className="w-11 h-11 rounded-md bg-gray-100 object-cover border border-gray-200 shadow-sm" loading="lazy" /><span className="text-[8px] text-navy-400 font-bold">كارنيه</span></div>
                    </div>
                  </div>
                ))}
              </div>
