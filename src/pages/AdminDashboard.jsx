...
export default function AdminDashboard() {
  ...
  const fetchAll = async () => {
    setLoading(true);
    try {
      const [{ data: logsData, error: logsErr }, { data: lecsData }] = await Promise.all([
        supabase.from('attendance_logs').select(`*, profiles:student_id (full_name, email, role), lectures:lecture_id (title, lecture_date, start_time, end_time)`).order('created_at', { ascending: false }),
        supabase.from('lectures').select('id, title, lecture_date').order('lecture_date', { ascending: false }),
      ]);
      ...
      const todayLogs = (logsData || []).filter(l => new Date(l.created_at).toDateString() === today);
      const uniqueStudents = new Set((logsData || []).map(l => l.student_id));
      setStats({ total: (logsData || []).length, today: todayLogs.length, students: uniqueStudents.size });
  ...
  const filteredLogs = logs.filter(log => {
    if (searchTerm) {
      ...
      if (!log.profiles?.full_name?.toLowerCase().includes(t) && !log.profiles?.email?.toLowerCase().includes(t)) return false;
    }
    if (filterLecture && log.lecture_id !== filterLecture) return false;
    if (filterToday && new Date(log.created_at).toDateString() !== new Date().toDateString()) return false;
    if (filterDateFrom) {
  ...
  const clearFilters = () => { setSearchTerm(''); setFilterLecture(''); setFilterDateFrom(''); setFilterDateTo(''); setFilterToday(false); };
  ...
  return (
    <div className="space-y-4 sm:space-y-5">
      ...
      {message.text && (
        <div className={`p-3 rounded-lg font-bold border text-xs ${message.type === 'success' ? 'bg-green-50 text-green-700 border-green-200' : 'bg-red-50 text-red-700 border-red-200'}`}>{message.text}</div>
      )}

      {/* Attendance Logs with Filters */}
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        {/* Filters */}
        <div className="p-3 sm:p-4 border-b border-gray-200 bg-navy-50 space-y-3">
                ...
                {filteredLogs.map(log => (
                  <tr key={log.id} className="hover:bg-navy-50/50 transition-colors">
                    ...
                    <td className="px-3 py-2.5">
                      {log.lectures ? (
                        <span className="text-[10px] font-bold bg-gold-400/10 text-navy-800 px-1.5 py-0.5 rounded border border-gold-400/30">{log.lectures.title}</span>
                      ) : (
                    ...
                    <td className="px-3 py-2.5"><span className="text-[10px] font-bold text-navy-700 bg-navy-50 px-1.5 py-1 rounded border border-navy-100" dir="ltr">{formatDate(log.created_at)}</span></td>
                    {isAdmin && (
