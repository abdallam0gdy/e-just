...
export default function LectureManager() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    setMsg({ type: '', text: '' });
    if (form.start_time >= form.end_time) { setMsg({ type: 'error', text: 'وقت البداية يجب أن يكون قبل وقت النهاية' }); return; }
    ...
    try {
      ...
      if (editing) {
        ...
        setMsg({ type: 'success', text: 'تم تعديل المحاضرة بنجاح' });
      } else {
        ...
        setMsg({ type: 'success', text: form.is_recurring ? 'تم إضافة المحاضرة وتكرارها لـ 4 أسابيع قادمة' : 'تم إضافة المحاضرة بنجاح' });
      }
      resetForm();
      fetchAll();
  ...
  const deleteLecture = async (id) => {
    if (!confirm('حذف المحاضرة نهائياً؟')) return;
    try {
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        ...
        <form onSubmit={handleSubmit} className="p-3 sm:p-4 border-b border-gray-200 bg-gray-50 space-y-3">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div className="sm:col-span-2">
              <label className="block text-[11px] font-bold text-navy-700 mb-1">اسم المحاضرة *</label>
              <input type="text" className="w-full px-3 py-2 rounded-lg border border-gray-200 focus:border-navy-600 outline-none text-sm" placeholder="مثال: فيزياء 101 - محاضرة 3" value={form.title} onChange={e => setForm({ ...form, title: e.target.value })} required />
            </div>
            <div className="sm:col-span-2">
              <label className="block text-[11px] font-bold text-navy-700 mb-1">وصف (اختياري)</label>
              <input type="text" className="w-full px-3 py-2 rounded-lg border border-gray-200 focus:border-navy-600 outline-none text-sm" placeholder="وصف مختصر" value={form.description} onChange={e => setForm({ ...form, description: e.target.value })} />
            </div>
            <div>
          ...
          {!editing && (
            <label className="flex items-center gap-2 cursor-pointer bg-white p-2.5 rounded-lg border border-gray-200">
              ...
              <span className="text-xs font-bold text-navy-800">تكرار أسبوعي (تتكرر لمدة 4 أسابيع قادمة)</span>
            </label>
          )}
          <div className="flex gap-2">
            <button type="submit" className="bg-navy-900 hover:bg-navy-800 text-white font-bold py-2 px-4 rounded-lg text-xs transition-colors">
              {editing ? 'حفظ التعديل' : 'إضافة المحاضرة'}
            </button>
            <button type="button" onClick={resetForm} className="bg-gray-100 hover:bg-gray-200 text-gray-700 font-bold py-2 px-4 rounded-lg text-xs transition-colors">إلغاء</button>
          </div>
        </form>
      )}
      ...
      {/* Table */}
      {loading ? (
        <div className="py-10 text-center"><div className="spinner mx-auto mb-2"></div><p className="text-xs text-navy-400">جاري التحميل...</p></div>
      ) : lectures.length === 0 ? (
        <div className="py-10 text-center text-navy-400"><p className="text-sm font-bold">لا توجد محاضرات</p><p className="text-xs mt-1">أضف محاضرة جديدة للبدء</p></div>
      ) : (
          ...
          <table className="w-full text-right border-collapse min-w-[550px]">
            <thead>
              <tr className="bg-navy-50 border-b border-navy-100">
                ...
                <th className="px-3 py-2.5 text-[11px] font-bold text-navy-600">التاريخ</th>
                <th className="px-3 py-2.5 text-[11px] font-bold text-navy-600">الوقت</th>
                <th className="px-3 py-2.5 text-[11px] font-bold text-navy-600 text-center">الحالة</th>
                <th className="px-3 py-2.5 text-[11px] font-bold text-navy-600 text-center">تحكم</th>
              </tr>
            </thead>
